###reproduce simulation results in Tables 1 and 2 (or Tables S1-S2), all methods are converged N times
###author: Zhiqiang CAO
rm(list = ls(all = TRUE))
...
#if we set n=9000 or 20000, we can obtain estimation results in Table S1 
n = 3000 
#simulation times
N = 1000
...
if(case == 1){
  ...
  po = 0.22
  pi0 = 0.14; pi1 = 0.23  #Pr(V=0|V_o=0)=0.963; Pr(V=1|V_o=1)=0.444
}else if(case == 3){
  po = 0.44
...
while (i<=N) {
  cat("iter=",i,"\n")
  set.seed(seed0[i]+k)
  #For Table S4: when choice=1 and case=1; using set.seed(100000+k);
  #              when choice=1 and case=3; using set.seed(seed0[i]+k)
  #              when choice=2, for both case 1 and case 3; using set.seed(seed0+k)
  #For Table S5: using set.seed(seed0[i]+k)
  #dtaset = dataset(n, pt, pi0, pi1)      #to generate simulated data in Tables 1-2, Tables S1-S2
...
#keep results
write.csv(ress,paste0("choice_",choice,"_case_",case,"_cor_",rho_uu,"_n_",n,".csv"))
